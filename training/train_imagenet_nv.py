...
# args.local_rank comes from the launch utility it makes sure 
# that local rank is the gpu number
...
def main():
    ...
    # model params is a list of parameters in fp16
    # master_params is a list of parameters in fp32 it's detached too, so no
    # grad calculation
    # there seems to be a bug in this, because this is called on model which is
    # already fp16 because  of line 102, but let's go on!!
    if args.fp16: model_params, master_params = prep_param_lists(model)
    else: model_params = master_params = model.parameters()
    # this gives out dictionary so batch norm parameter run without weight
    # decay while rest of model has weight decay
    ...
    # some data managment based on learning rate schedule
...
def train(trn_loader, model, criterion, optimizer, scheduler, epoch):
    ...
    for i,(input,target) in enumerate(trn_loader):
        ...
        if args.fp16:
            ...
            optimizer.zero_grad()
            loss.backward()
            # atomo implementation for non fp16 case
            for name, param in model.named_parameters():
                grad_val = param.grad.data
                # converting grad val to cpu to use with current version of
                grad_val = grad_val.cpu().numpy()
